...
export function ProcessSection({ locale = "es" }: { locale?: "en" | "es" }) {
  const copy = locale === "en"
    ? { eyebrow: "Process", title: "How I work", description: "A simple process for turning a need into a solution that works.", steps: [{ number: "01", title: "Understand the problem", description: "We talk through your context, goals, and the people who will use the solution." }, { number: "02", title: "Design the solution", description: "We organize the ideas and define a clear, useful solution that can grow." }, { number: "03", title: "Development", description: "I build the product with visible deliveries and constant communication." }, { number: "04", title: "Launch and improve", description: "We launch, measure what matters, and keep improving based on real needs." }] }
    : { eyebrow: "Proceso", title: "Cómo trabajo", description: "Un proceso simple para convertir una necesidad en una solución que funcione.", steps };
      ...
      <Container>
        ...
        <div className="relative mt-12 grid gap-8 md:grid-cols-4 md:gap-5">
          <div className="absolute left-0 right-0 top-0 hidden h-px bg-line md:block">
            <div className="motion-process-line h-full w-full bg-accent" />
          </div>
          {copy.steps.map((step) => (
            <article key={step.number} className="relative border-l border-line pl-5 md:border-l-0 md:border-t md:pl-0 md:pt-8">
              <span className="absolute -left-[5px] top-5 h-2.5 w-2.5 rounded-full bg-accent ring-4 ring-canvas md:left-0 md:top-[-5px]" />
