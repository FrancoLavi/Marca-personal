...
export function ProcessSection() {
  return <section className="border-b border-line py-20 sm:py-28"><Container><SectionHeading eyebrow="Proceso" title="Cómo trabajo" description="Un proceso simple para convertir una necesidad en una solución que funcione." /><div className="mt-12 grid gap-8 md:grid-cols-4 md:gap-5">{steps.map((step) => <article key={step.number} className="relative border-l border-line pl-5 md:border-l-0 md:border-t md:pl-0 md:pt-5"><span className="text-sm font-semibold text-accent">{step.number}</span><h3 className="mt-4 text-lg font-semibold text-ink">{step.title}</h3><p className="mt-3 text-sm leading-6 text-muted">{step.description}</p></article>)}</div></Container></section>;
}
