...
export function ServicesListSection({ locale = "es" }: { locale?: "en" | "es" }) {
  ...
  return (
    <section className="py-20 sm:py-28">
      <Container>
        <SectionHeading eyebrow={en ? "Solutions" : "Soluciones"} title={en ? "A solution for every challenge" : "Una solución para cada desafío"} description={en ? "Each service can be adapted to your project's stage, size, and goals." : "Cada servicio puede adaptarse al momento, tamaño y objetivos de tu proyecto."} />
        <div className="mt-12 grid gap-5 lg:grid-cols-2">{services.map((service, index) => <ServiceDetailCard key={service.title} service={service} featured={index === 0} locale={locale} />)}</div>
      </Container>
    </section>
  );
}
...
function ServiceDetailCard({ service, featured, locale }: { service: Service; featured: boolean; locale: "en" | "es" }) {
  ...
  return (
    <article className={`rounded-xl border border-line p-6 sm:p-8 ${featured ? "bg-ink text-white lg:col-span-2" : "bg-card"}`}>
      <div className="flex flex-col gap-7 sm:flex-row sm:justify-between">
        <div className="max-w-xl">
          <div className={featured ? "text-accent" : "text-accent"}><ServiceIcon name={service.icon} /></div>
          <p className={`mt-6 font-mono text-xs font-semibold uppercase tracking-[0.16em] ${featured ? "text-white/45" : "text-muted"}`}>0{services.indexOf(service) + 1}</p>
          <h2 className={`mt-2 text-2xl font-semibold tracking-[-0.03em] ${featured ? "text-white" : "text-ink"}`}>{copy.title}</h2>
          <p className={`mt-4 max-w-xl text-base leading-7 ${featured ? "text-white/65" : "text-muted"}`}>{copy.description}</p>
        </div>
        <div className={`sm:min-w-[240px] sm:max-w-[280px] ${featured ? "text-white/75" : "text-muted"}`}>
          <p className={`font-mono text-xs font-semibold uppercase tracking-[0.16em] ${featured ? "text-accent" : "text-accent"}`}>{en ? "Examples" : "Ejemplos"}</p>
          <ul className="mt-4 space-y-2.5 text-sm">{copy.examples.map((example) => <li key={example} className="flex gap-2"><span className="text-accent" aria-hidden="true">•</span><span>{example}</span></li>)}</ul>
        </div>
      </div>
    </article>
  );
